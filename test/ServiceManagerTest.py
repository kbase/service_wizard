...
# Catalog Responses
catalog_get_module_version_response = {'module_name': MODULE_NAME, 'released': 0, 'released_timestamp': None,
                                       'notes': '', 'timestamp': 1603397275959,
                                       ...
                                       'release_tags': ['beta', 'dev'], 'release_timestamp': None}
catalog_secure_vars_response = [
    {'module_name': MODULE_NAME, 'version': '', 'param_name': 'service_token1',
     'param_value': 'password123', 'is_password': 1},
    {'module_name': MODULE_NAME, 'version': '',
     'param_name': 'admin_token1',
     'param_value': 'password321', 'is_password': 1}]
catalog_volume_mounts_response = [
    {'module_name': MODULE_NAME, 'function_name': 'service', 'client_group': 'service', 'volume_mounts': [
        {'host_dir': '/data/static_narratives', 'container_dir': '/kb/module/work/nginx', 'read_only': 0}]}]
...
def sw():
    ...
    sw.catalog_admin_client.get_module_version.return_value = catalog_get_module_version_response
    sw.catalog_admin_client.get_secure_config_params.return_value = catalog_secure_vars_response
    sw.catalog_admin_client.list_volume_mounts.return_value = catalog_volume_mounts_response
...
def test_start_service_catalog_call(sw):
    ...
    sw.catalog_admin_client.get_module_version.assert_called_with({'module_name': MODULE_NAME, 'version': VERSION_TAG})
